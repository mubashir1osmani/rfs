...
enum SeedData {
    ...
    static var calendar: [CalendarBlock] {
        [
            block(id: "event-1", title: "Deep work", hour: 9, minute: 0, duration: 90, color: "mint"),
            block(id: "event-2", title: "Team standup", hour: 11, minute: 0, duration: 30, color: "violet"),
            block(id: "event-3", title: "Lunch reset", hour: 12, minute: 30, duration: 45, color: "yellow"),
        ]
    }
    ...
    private static func block(id: String, title: String, hour: Int, minute: Int, duration: Int, color: String) -> CalendarBlock {
        let start = Calendar.current.date(bySettingHour: hour, minute: minute, second: 0, of: Date()) ?? Date()
        return CalendarBlock(id: id, title: title, start: start, durationMinutes: duration, colorName: color, source: .seed, attendees: [])
    }
}
